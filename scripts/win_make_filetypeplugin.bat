@echo off
rem Build script for the FileTypePlugin subproject on Windows using MSVC and JOM.
rem Builds libfile, liboniguruma, and FileTypePlugin in both debug and release
rem configurations unless NO_DEBUG=1 or NO_RELEASE=1 is set.
rem
...
rem Build debug configurations unless NO_DEBUG is set.
if "%NO_DEBUG%"=="" (
...
rem Build release configurations unless NO_RELEASE is set.
if "%NO_RELEASE%"=="" (
...
rem   %1 = project name (e.g. FileTypePlugin)
