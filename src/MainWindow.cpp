...
{
	{
		...
		while (i > 0) {
			...
			if (m->event_item_list[i].at <= now) { // 予約時間を過ぎていたら
				...
				m->event_item_list.erase(m->event_item_list.begin() + i); // 処理待ちリストから削除
			}
		}
...
void MainWindow::reopenRepository(bool log, const std::function<void (const GitPtr &)> &callback)
{
	GitPtr g = git();
	if (!isValidWorkingCopy(g)) return;
...
void MainWindow::queryBranches(RepositoryWrapperFrame *frame, const GitPtr &g)
{
	Q_ASSERT(g);
...
void MainWindow::queryRemotes(const GitPtr &g)
{
	if (!g) return;
...
void MainWindow::setLogEnabled(const GitPtr &g, bool f)
{
	if (f) {
...
Git::Object MainWindow::cat_file_(RepositoryWrapperFrame *frame, const GitPtr &g, const QString &id)
{
	if (isValidWorkingCopy(g)) {
...
bool MainWindow::fetch(const GitPtr &g, bool prune)
{
...
bool MainWindow::fetch_tags_f(const GitPtr &g)
{
	setPtyCondition(PtyCondition::Fetch);
	setPtyProcessOk(true);
...
Git::CommitItemList MainWindow::retrieveCommitLog(const GitPtr &g)
{
...
void MainWindow::updateWindowTitle(const GitPtr &g)
{
	if (isValidWorkingCopy(g)) {
		Git::User user = g->getUser(Git::Source::Default);
		setWindowTitle_(user);
