...
FileViewType FileDiffWidget::setupPreviewWidget()
{
	...
	std::string mimetype_l = mainwindow()->determinFileType(m->init_param_.bytes_a);
	std::string mimetype_r = mainwindow()->determinFileType(m->init_param_.bytes_b);
...
void FileDiffWidget::updateDiffView(Git::Diff const &info, bool uncommited)
{
	...
	if (isValidID_(info.blob.a_id_or_path) && isValidID_(info.blob.b_id_or_path)) {
		...
		std::string mime_a = mainwindow()->determinFileType(obj_a.content);
		std::string mime_b = mainwindow()->determinFileType(obj_b.content);
