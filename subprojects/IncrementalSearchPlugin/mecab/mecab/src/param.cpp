...
#ifndef USE_CUSTOM_DICTIONARY_LOADER
...
std::vector<std::string> split_lines(char const *begin, char const *end)
{
	std::vector<std::string> lines;
	char const *left = begin;
	char const *right = begin;
	while (1) {
		char const *last = right;
		int c = 0;
		if (right < end) {
		...
		if (c == '\n' || c == '\r' || c == 0) {
			if (right < end && *right == '\r') {
				right++;
			}
			lines.push_back(std::string(left, last));
			if (c == 0) break;
			left = right;
		}
	}
...
bool Param::load(const char *filename)
{
	...
	std::vector<std::string> lines;
	std::vector<char> buf;
	if (x_get_resource_i8(filename, &buf)) {
		char const *begin = buf.data();
		char const *end = begin + buf.size();
		lines = split_lines(begin, end);
	} else {
...
#define GOTO_ERROR(n) \
	...
	for (size_t i = 0; opts[i].name; ++i) {
		if (opts[i].default_value) set<std::string>(opts[i].name, opts[i].default_value);
	}
