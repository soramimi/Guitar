...
function recordsURL(params) {
  ...
  q.set('file', currentFile);
  return '/api/records?' + q.toString();
}

function recordURL(id) {
  var q = new URLSearchParams();
  q.set('file', currentFile);
  return '/api/records/' + encodeURIComponent(id) + '?' + q.toString();
}

function saveURL() {
  var q = new URLSearchParams();
  q.set('file', currentFile);
  return '/api/save?' + q.toString();
}
...
function fetchPage(page, query) {
  ...
  var url = recordsURL({ page: curPage, per_page: perPage, q: curQuery });
...
function mkDel(id) {
  return function () {
    ...
    fetch(recordURL(id), { method: 'DELETE' }).then(function (r) {
      if (r.ok) {
        fetchPage(curPage);
      } else {
...
function showFetchError(label, r) {
  return r.text().then(function (text) {
    var detail = text ? ': ' + text.trim() : '';
    setStatus(label + ' (' + r.status + ')' + detail, true);
    return null;
  });
}
...
function addRow() {
  ...
  fetch(recordsURL(), {
    ...
    if (!r.ok) return showFetchError('追加エラー', r);
    return r.json();
  }).catch(function (err) {
    setStatus('追加エラー: ' + err.message, true);
    return null;
  }).then(function (rec) {
    if (!rec) return;
...
function saveFile() {
  setStatus('保存中...', false);
  fetch(saveURL(), { method: 'POST' }).then(function (r) {
    if (r.ok) {
      location.href = '/';
    } else {
